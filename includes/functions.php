...
function cpcf_purge_everything( $reason = '' ) {
	if ( '' === $reason ) {
		$reason = __( 'Purge everything requested by code', 'cache-purge-control-for-cloudflare' );
	}
...
function cpcf_purge_urls( array $urls, $reason = '' ) {
	if ( '' === $reason ) {
		$reason = __( 'URL purge requested by code', 'cache-purge-control-for-cloudflare' );
	}
...
function cpcf_purge_post( $post, $reason = '' ) {
	...
	if ( '' === $reason ) {
		...
		$reason = sprintf( __( 'Purge requested by code for "%s"', 'cache-purge-control-for-cloudflare' ), $post->post_title );
	}
